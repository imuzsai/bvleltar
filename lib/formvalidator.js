...
signupFormCheck: function(req,res,validator){
    ...
    req.checkBody("password", "Jelszó min. 8 max. 15 karakter (szóköz és ékezet nélkül), 1 szám, 1 speciális karakter (@#$%^+-) !").isLength({ min: 8,max:15 }).matches('^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^+-])').matches('^[a-zA-Z0-9@#$%^+-]+$');
    req.checkBody("password2", "Jelszó-újra min. 8 max. 15 karakter (szóköz és ékezet nélkül), 1 szám, 1 speciális karakter (@#$%^+-) !").isLength({ min: 8,max:15 }).matches('^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^+-])').matches('^[a-zA-Z0-9@#$%^+-]+$');
